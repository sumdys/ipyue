...
function result($msg='',$status=true,$extra=array(),$json=true) {
	...
	//$arr['user_id'] = C('USER_ID');
	//$arr['request_method'] = REQUEST_METHOD;
	//$arr['new_token'] = C('NEW_TOKEN');
	I('param.show_sql') == 'true' && !$arr['sql'] && $arr['sql'] = debug_sql();
	if (I('param.show_time') == 'true') {
		constant('APP_DEBUG') && $arr['elapsed_time'] = 
		round((microtime(true) - NOW_MICROTIME),3);
	}
...
function error($msg='',$extra=array()) {
	return result($msg,false,$extra,true);
}
?>