...
class RequireOrderModel extends RelationModel{
    ...
    function insert($data){
        ...
        $map['phone']=I('phone');
        $map['create_time']=array('gt',time()-10);
        $rs=$this->where($map)->count();
        if($rs){
            $this->error="发表的太频繁，休息一下";
            return false;
        }
        //重复发表
   ...
   */
    function wordFilter($string){
        ...
        $Filter=$this->badkey.$this->badkey2;
