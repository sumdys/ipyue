...
//		$map['_complex'] = $where;	
    ...
    function add_cash_pay_money(){
        ...
        if($_POST){
            $where['id']=I('post.id');
            $saveData['cash_pay_money']=sprintf('%0.2f',trim(I('post.cash_pay_money')));
            if(!$m->where($where)->save($saveData)){
                $this->error('录入失败');
            }
            $this->success('录入成功');
        }else{
            $where['id']=$id;
            $orderData = $m->where($where)->field('id,order_num')->find();
