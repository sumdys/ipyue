...
class PayAction extends IniAction {
    ...
    function createOrder(){
        if(session('pay_auth')){
            ...
            $data['id']=$pay_auth['order_pay_id'];
            $data['order_id_arr']=$pay_auth['order_id_arr'];
            $data['order_price']=$pay_auth['total_price'];
    ...
    function pay_auth(){
        $pay_auth=session('pay_auth');
        if(I('order_no')!=$pay_auth['order_pay_id'] || I('order_price')!=$pay_auth['total_price']){
	...
	****************************/		
	...
	function send(){
		...
		$bgUrl = "http://www.ipyue.com/Member/pay/recieve";
							...
							if($_REQUEST[payAmount] == $_REQUEST[orderAmount]){								
								...
								if($rs){
									...
									if($res){
										...
										echo '<redirecturl>http://www.ipyue.com/Member/booking?status=process</redirecturl>';
									}
								}								
							}
					default:
							echo '<result>1</result>';
							echo '<redirecturl>http://www.ipyue.com/Member/booking?status=pending</redirecturl>';								
			}	
		}else{	
			echo '<result>1</result>';
			echo '<redirecturl>http://www.ipyue.com/Member/booking?status=pending</redirecturl>';								
