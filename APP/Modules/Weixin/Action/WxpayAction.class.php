...
//    }
    ...
    public function pay()
    {
        ...
        //取ORDER
        $cache = D('TripOrder')->where(array('oid' => $oid))->find();
        if (!$cache) {
            $this->error('此订单不存在！', U('App/Shop/index'));
			die();
		}
        //微信支付封装
        $options['appid'] = self::$SET['wxappid'];
        $options['appsecret'] = self::$SET['wxappsecret'];
        $options['mchid'] = self::$SET['wxmchid'];
        $options['mchkey'] = self::$SET['wxmchkey'];
        ini_set('date.timezone','Asia/Shanghai');
//error_reporting(E_ERROR);
//        require_once "../../Lib/ORG/Wxpay/WxPay.Api.php";
//        require_once "../../Lib/ORG/Wxpay/WxPay.JsApiPay.php";
		$paysdk = new JsApiPay();  echo 222;exit;
        $openid = $paysdk->GetOpenid();
        ...
        $paysdk->setParameter("total_fee", intval($cache['total_price'] * 100)); //总金额单位为分，不允许有小数
        $paysdk->setParameter("notify_url", 'http://' . $_SERVER['HTTP_HOST'] .'/wxpay.php'); //交易通知地址
        $paysdk->setParameter("trade_type", "JSAPI"); //交易类型
        $prepayid = $paysdk->getPrepayId();
//		logs($prepayid);
//		logs('http://' . $_SERVER['HTTP_HOST'] . __ROOT__ . U('Home/Wxpay/nd'));
        if ($prepayid) {
...
//var_dump($paysdk->getJSAPI());die();
print <<<EOT
<script type="text/javascript">
document.addEventListener('WeixinJSBridgeReady', function onBridgeReady() {
        WeixinJSBridge.invoke('getBrandWCPayRequest', 
EOT;
echo json_encode($paysdk->getJSAPI());
print <<<EOT
		, function(res) {
                if(res.err_msg == 'get_brand_wcpay_request:ok') {
                  //      alert('您已成功付款！ ');
                        window.location.href='http://{$_SERVER[HTTP_HOST]}/index.php?m=App&c=Shop&a=orderList&sid=0';
                } else {
...
//        header("Location:" . $url);
    ...
    {
        $url = self::$_url . U('App/Shop/orderList',Array('sid' => $_SESSION['wxpaysid']));
    ...
    public function payback()
    {
        ...
        if ($verify_result) {
            ...
            if ($result == 'success') {
                ...
                $url = self::$_url . U('App/Shop/orderList',Array("sid"=>self::$_opt['sid']));//'/App/Shop/orderList/sid/' . self::$_opt['sid'];
                header('Location:' . $url);
            } else {
                echo "支付失败"; //这里永远不会调用
                $url = self::$_url . U('App/Shop/orderList',Array("sid"=>self::$_opt['sid']));
                header('Location:' . $url);
            }
    ...
    //付款成功后操作
    public function endpay($oid, $buyer_email=null)
    {
        $m = M('Shop_order');
        $dwechat = D('Wechat');
        ...
        if ($order) {
            if ($order['status'] == 1) {
                ...
                if (FALSE !== $re) {
                    //扣除会员积分哦
                    if ($order["totalpoint"]>0)
                    {
                        $sql="UPDATE __VIP__ SET score=score-".$order['totalpoint']." WHERE id=".$order["vipid"];
                        $m->execute($sql);
                    }
                    //销量计算-只减不增
