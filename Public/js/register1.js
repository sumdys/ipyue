...
$(function(){
	$("#qiehuan_qb").click(function(){
        ...
        $("#reg_id").blur(function(){
            ...
            var pro=$(this).next();
            var procont="";
            procont=value==""?"不能为空":re2.test(value)?"开始或结束不能含空字符":value.length<5?"不能小于5个字符":value.length>30?"不能大于30个字符":(!re1.test(value))?"不能含中文或个别符号":"";
            if(procont!=""){$(this).parent().attr("val","1");pro.html("<img src='"+js_path+"alterError.gif' /><a>"+ggg+"</a>");}
            else{
				//验证调用
