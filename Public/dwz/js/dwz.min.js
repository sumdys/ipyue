        ...
        genTree: function(options) {
            ...
            return this.each(function(){
                ...
                if (tree.size() > 0) {
                    ...
                    if(node.children().children().hasClass('node')){
                     //   alert(node.html());
                    }else{
...
var navTab = {
    ...
    _switchTab: function(iTabIndex) {
        ...
        var tabid_s = this._getTabs().eq(iTabIndex).attr('tabid');
        if(tabid_s){
        setTimeout(function(){
                $.History.addHistory(tabid_s,
                    function(tabid_s) {
                        var i = navTab._indexTabId(tabid_s);
    ...
    $.extend({
        bringBackSuggest: function(args) {
            ...
            $box.find(":input").each(function() {
                var $input = $(this);
                if($input.attr("lookup")){  //lookup属性 代替name
                    inputName = $input.attr("lookup");
                }else{
...
// 因为通过JS创建新的日历控件后点击事件无效，因此在外面创建了一个日历重载函数
function datepickerOverLoaded(){
